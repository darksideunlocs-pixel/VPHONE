.class public Lcom/umeng/social/tool/ComposeTool;
.super Ljava/lang/Object;
.source "ComposeTool.java"


# annotations
.annotation system Ldalvik/annotation/MemberClasses;
    value = {
        Lcom/umeng/social/tool/ComposeTool$ComposeDirection;
    }
.end annotation


# static fields
.field public static backgroundColor:I

.field public static direction:Lcom/umeng/social/tool/ComposeTool$ComposeDirection;

.field public static textColor:I

.field public static textsize:I

.field public static typeface:Landroid/graphics/Typeface;


# direct methods
.method static constructor <clinit>()V
    .locals 1

    .line 18
    sget-object v0, Lcom/umeng/social/tool/ComposeTool$ComposeDirection;->CUSTOM:Lcom/umeng/social/tool/ComposeTool$ComposeDirection;

    sput-object v0, Lcom/umeng/social/tool/ComposeTool;->direction:Lcom/umeng/social/tool/ComposeTool$ComposeDirection;

    const/high16 v0, -0x1000000

    .line 19
    sput v0, Lcom/umeng/social/tool/ComposeTool;->textColor:I

    const/16 v0, 0x12

    .line 20
    sput v0, Lcom/umeng/social/tool/ComposeTool;->textsize:I

    const/4 v0, -0x1

    .line 21
    sput v0, Lcom/umeng/social/tool/ComposeTool;->backgroundColor:I

    .line 22
    sget-object v0, Landroid/graphics/Typeface;->DEFAULT:Landroid/graphics/Typeface;

    sput-object v0, Lcom/umeng/social/tool/ComposeTool;->typeface:Landroid/graphics/Typeface;

    return-void
.end method

.method public constructor <init>()V
    .locals 0

    .line 16
    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    return-void
.end method

.method public static createCompose(Landroid/graphics/Bitmap;Landroid/graphics/Bitmap;ZI)Landroid/graphics/Bitmap;
    .locals 6

    const/4 v0, 0x0

    if-nez p0, :cond_0

    return-object v0

    :cond_0
    if-nez p1, :cond_1

    return-object v0

    .line 31
    :cond_1
    invoke-virtual {p0}, Landroid/graphics/Bitmap;->getWidth()I

    move-result v1

    .line 32
    invoke-virtual {p0}, Landroid/graphics/Bitmap;->getHeight()I

    move-result v2

    .line 33
    invoke-virtual {p1}, Landroid/graphics/Bitmap;->getWidth()I

    move-result v3

    .line 34
    invoke-virtual {p1}, Landroid/graphics/Bitmap;->getHeight()I

    move-result v4

    if-eqz p2, :cond_2

    .line 35
    invoke-static {v1, v3}, Ljava/lang/Math;->max(II)I

    move-result v3

    goto :goto_0

    :cond_2
    add-int/2addr v3, v1

    add-int/2addr v3, p3

    :goto_0
    if-eqz p2, :cond_3

    add-int/2addr v4, v2

    add-int/2addr v4, p3

    goto :goto_1

    .line 36
    :cond_3
    invoke-static {v2, v4}, Ljava/lang/Math;->max(II)I

    move-result v4

    .line 37
    :goto_1
    sget-object v5, Landroid/graphics/Bitmap$Config;->ARGB_8888:Landroid/graphics/Bitmap$Config;

    invoke-static {v3, v4, v5}, Landroid/graphics/Bitmap;->createBitmap(IILandroid/graphics/Bitmap$Config;)Landroid/graphics/Bitmap;

    move-result-object v3

    .line 38
    new-instance v4, Landroid/graphics/Canvas;

    invoke-direct {v4, v3}, Landroid/graphics/Canvas;-><init>(Landroid/graphics/Bitmap;)V

    const/4 v5, 0x0

    .line 39
    invoke-virtual {v4, p0, v5, v5, v0}, Landroid/graphics/Canvas;->drawBitmap(Landroid/graphics/Bitmap;FFLandroid/graphics/Paint;)V

    if-eqz p2, :cond_4

    add-int/2addr v2, p3

    int-to-float p0, v2

    .line 41
    invoke-virtual {v4, p1, v5, p0, v0}, Landroid/graphics/Canvas;->drawBitmap(Landroid/graphics/Bitmap;FFLandroid/graphics/Paint;)V

    return-object v3

    :cond_4
    add-int/2addr v1, p3

    int-to-float p0, v1

    .line 43
    invoke-virtual {v4, p1, p0, v5, v0}, Landroid/graphics/Canvas;->drawBitmap(Landroid/graphics/Bitmap;FFLandroid/graphics/Paint;)V

    return-object v3
.end method

.method public static createTextImage(Ljava/lang/String;Landroid/graphics/Bitmap;II)Landroid/graphics/Bitmap;
    .locals 11

    .line 84
    invoke-virtual {p1}, Landroid/graphics/Bitmap;->getConfig()Landroid/graphics/Bitmap$Config;

    move-result-object v0

    .line 85
    invoke-virtual {p1}, Landroid/graphics/Bitmap;->getWidth()I

    move-result v4

    .line 86
    invoke-virtual {p1}, Landroid/graphics/Bitmap;->getHeight()I

    move-result v9

    if-nez v0, :cond_0

    .line 90
    sget-object v0, Landroid/graphics/Bitmap$Config;->ARGB_8888:Landroid/graphics/Bitmap$Config;

    .line 92
    :cond_0
    new-instance v3, Landroid/text/TextPaint;

    const/4 v10, 0x1

    invoke-direct {v3, v10}, Landroid/text/TextPaint;-><init>(I)V

    .line 93
    sget v1, Lcom/umeng/social/tool/ComposeTool;->textColor:I

    invoke-virtual {v3, v1}, Landroid/text/TextPaint;->setColor(I)V

    .line 94
    sget v1, Lcom/umeng/social/tool/ComposeTool;->textsize:I

    int-to-float v1, v1

    invoke-virtual {v3, v1}, Landroid/text/TextPaint;->setTextSize(F)V

    .line 95
    invoke-virtual {v3, v10}, Landroid/text/TextPaint;->setDither(Z)V

    .line 96
    invoke-virtual {v3, v10}, Landroid/text/TextPaint;->setFilterBitmap(Z)V

    .line 97
    sget-object v1, Lcom/umeng/social/tool/ComposeTool;->typeface:Landroid/graphics/Typeface;

    invoke-virtual {v3, v1}, Landroid/text/TextPaint;->setTypeface(Landroid/graphics/Typeface;)Landroid/graphics/Typeface;

    .line 99
    new-instance v1, Landroid/text/StaticLayout;

    sget-object v5, Landroid/text/Layout$Alignment;->ALIGN_NORMAL:Landroid/text/Layout$Alignment;

    const/4 v7, 0x0

    const/4 v8, 0x1

    const/high16 v6, 0x3f800000    # 1.0f

    move-object v2, p0

    invoke-direct/range {v1 .. v8}, Landroid/text/StaticLayout;-><init>(Ljava/lang/CharSequence;Landroid/text/TextPaint;ILandroid/text/Layout$Alignment;FFZ)V

    .line 100
    new-instance p0, Landroid/graphics/Rect;

    invoke-direct {p0}, Landroid/graphics/Rect;-><init>()V

    const/4 v5, 0x0

    .line 101
    invoke-virtual {v2}, Ljava/lang/String;->length()I

    move-result v6

    invoke-virtual {v3, v2, v5, v6, p0}, Landroid/text/TextPaint;->getTextBounds(Ljava/lang/String;IILandroid/graphics/Rect;)V

    .line 103
    invoke-virtual {p1, v0, v10}, Landroid/graphics/Bitmap;->copy(Landroid/graphics/Bitmap$Config;Z)Landroid/graphics/Bitmap;

    move-result-object p0

    mul-int/lit8 p1, p2, 0x2

    add-int/2addr v4, p1

    .line 104
    invoke-virtual {v1}, Landroid/text/StaticLayout;->getHeight()I

    move-result p1

    add-int/2addr v9, p1

    mul-int/lit8 p1, p3, 0x4

    add-int/2addr v9, p1

    sget-object p1, Landroid/graphics/Bitmap$Config;->ARGB_8888:Landroid/graphics/Bitmap$Config;

    invoke-static {v4, v9, p1}, Landroid/graphics/Bitmap;->createBitmap(IILandroid/graphics/Bitmap$Config;)Landroid/graphics/Bitmap;

    move-result-object p1

    .line 105
    new-instance v0, Landroid/graphics/Canvas;

    invoke-direct {v0, p1}, Landroid/graphics/Canvas;-><init>(Landroid/graphics/Bitmap;)V

    .line 106
    sget v2, Lcom/umeng/social/tool/ComposeTool;->backgroundColor:I

    invoke-virtual {v0, v2}, Landroid/graphics/Canvas;->drawColor(I)V

    int-to-float p2, p2

    .line 107
    invoke-virtual {v1}, Landroid/text/StaticLayout;->getHeight()I

    move-result v2

    mul-int/lit8 v3, p3, 0x3

    add-int/2addr v2, v3

    int-to-float v2, v2

    const/4 v3, 0x0

    invoke-virtual {v0, p0, p2, v2, v3}, Landroid/graphics/Canvas;->drawBitmap(Landroid/graphics/Bitmap;FFLandroid/graphics/Paint;)V

    int-to-float p0, p3

    .line 108
    invoke-virtual {v0, p2, p0}, Landroid/graphics/Canvas;->translate(FF)V

    .line 109
    invoke-virtual {v1, v0}, Landroid/text/StaticLayout;->draw(Landroid/graphics/Canvas;)V

    return-object p1
.end method

.method public static createWaterMask(Landroid/graphics/Bitmap;Landroid/graphics/Bitmap;II)Landroid/graphics/Bitmap;
    .locals 11

    const/4 v0, 0x0

    if-nez p0, :cond_0

    return-object v0

    .line 52
    :cond_0
    invoke-virtual {p0}, Landroid/graphics/Bitmap;->getWidth()I

    move-result v1

    .line 53
    invoke-virtual {p0}, Landroid/graphics/Bitmap;->getHeight()I

    move-result v2

    .line 54
    invoke-virtual {p1}, Landroid/graphics/Bitmap;->getWidth()I

    move-result v3

    .line 55
    invoke-virtual {p1}, Landroid/graphics/Bitmap;->getHeight()I

    move-result v4

    .line 56
    div-int/lit8 v5, v1, 0x2

    div-int/lit8 v6, v3, 0x2

    sub-int/2addr v5, v6

    .line 57
    div-int/lit8 v6, v2, 0x2

    div-int/lit8 v7, v4, 0x2

    sub-int/2addr v6, v7

    .line 58
    sget-object v7, Landroid/graphics/Bitmap$Config;->ARGB_8888:Landroid/graphics/Bitmap$Config;

    invoke-static {v1, v2, v7}, Landroid/graphics/Bitmap;->createBitmap(IILandroid/graphics/Bitmap$Config;)Landroid/graphics/Bitmap;

    move-result-object v7

    .line 59
    new-instance v8, Landroid/graphics/Canvas;

    invoke-direct {v8, v7}, Landroid/graphics/Canvas;-><init>(Landroid/graphics/Bitmap;)V

    const/4 v9, 0x0

    .line 60
    invoke-virtual {v8, p0, v9, v9, v0}, Landroid/graphics/Canvas;->drawBitmap(Landroid/graphics/Bitmap;FFLandroid/graphics/Paint;)V

    .line 61
    sget-object p0, Lcom/umeng/social/tool/ComposeTool;->direction:Lcom/umeng/social/tool/ComposeTool$ComposeDirection;

    sget-object v10, Lcom/umeng/social/tool/ComposeTool$ComposeDirection;->CUSTOM:Lcom/umeng/social/tool/ComposeTool$ComposeDirection;

    if-ne p0, v10, :cond_1

    int-to-float p0, p2

    int-to-float p2, p3

    .line 62
    invoke-virtual {v8, p1, p0, p2, v0}, Landroid/graphics/Canvas;->drawBitmap(Landroid/graphics/Bitmap;FFLandroid/graphics/Paint;)V

    return-object v7

    .line 63
    :cond_1
    sget-object p0, Lcom/umeng/social/tool/ComposeTool;->direction:Lcom/umeng/social/tool/ComposeTool$ComposeDirection;

    sget-object p2, Lcom/umeng/social/tool/ComposeTool$ComposeDirection;->UP:Lcom/umeng/social/tool/ComposeTool$ComposeDirection;

    if-ne p0, p2, :cond_2

    int-to-float p0, v5

    .line 64
    invoke-virtual {v8, p1, p0, v9, v0}, Landroid/graphics/Canvas;->drawBitmap(Landroid/graphics/Bitmap;FFLandroid/graphics/Paint;)V

    return-object v7

    .line 65
    :cond_2
    sget-object p0, Lcom/umeng/social/tool/ComposeTool;->direction:Lcom/umeng/social/tool/ComposeTool$ComposeDirection;

    sget-object p2, Lcom/umeng/social/tool/ComposeTool$ComposeDirection;->DOWN:Lcom/umeng/social/tool/ComposeTool$ComposeDirection;

    if-ne p0, p2, :cond_3

    int-to-float p0, v5

    sub-int/2addr v2, v4

    int-to-float p2, v2

    .line 66
    invoke-virtual {v8, p1, p0, p2, v0}, Landroid/graphics/Canvas;->drawBitmap(Landroid/graphics/Bitmap;FFLandroid/graphics/Paint;)V

    return-object v7

    .line 67
    :cond_3
    sget-object p0, Lcom/umeng/social/tool/ComposeTool;->direction:Lcom/umeng/social/tool/ComposeTool$ComposeDirection;

    sget-object p2, Lcom/umeng/social/tool/ComposeTool$ComposeDirection;->LEFT:Lcom/umeng/social/tool/ComposeTool$ComposeDirection;

    if-ne p0, p2, :cond_4

    int-to-float p0, v6

    .line 68
    invoke-virtual {v8, p1, v9, p0, v0}, Landroid/graphics/Canvas;->drawBitmap(Landroid/graphics/Bitmap;FFLandroid/graphics/Paint;)V

    return-object v7

    .line 69
    :cond_4
    sget-object p0, Lcom/umeng/social/tool/ComposeTool;->direction:Lcom/umeng/social/tool/ComposeTool$ComposeDirection;

    sget-object p2, Lcom/umeng/social/tool/ComposeTool$ComposeDirection;->RIGHT:Lcom/umeng/social/tool/ComposeTool$ComposeDirection;

    if-ne p0, p2, :cond_5

    sub-int/2addr v1, v3

    int-to-float p0, v1

    int-to-float p2, v6

    .line 70
    invoke-virtual {v8, p1, p0, p2, v0}, Landroid/graphics/Canvas;->drawBitmap(Landroid/graphics/Bitmap;FFLandroid/graphics/Paint;)V

    return-object v7

    .line 71
    :cond_5
    sget-object p0, Lcom/umeng/social/tool/ComposeTool;->direction:Lcom/umeng/social/tool/ComposeTool$ComposeDirection;

    sget-object p2, Lcom/umeng/social/tool/ComposeTool$ComposeDirection;->LEFTUP:Lcom/umeng/social/tool/ComposeTool$ComposeDirection;

    if-ne p0, p2, :cond_6

    .line 72
    invoke-virtual {v8, p1, v9, v9, v0}, Landroid/graphics/Canvas;->drawBitmap(Landroid/graphics/Bitmap;FFLandroid/graphics/Paint;)V

    return-object v7

    .line 73
    :cond_6
    sget-object p0, Lcom/umeng/social/tool/ComposeTool;->direction:Lcom/umeng/social/tool/ComposeTool$ComposeDirection;

    sget-object p2, Lcom/umeng/social/tool/ComposeTool$ComposeDirection;->LEFTDOWN:Lcom/umeng/social/tool/ComposeTool$ComposeDirection;

    if-ne p0, p2, :cond_7

    sub-int/2addr v2, v4

    int-to-float p0, v2

    .line 74
    invoke-virtual {v8, p1, v9, p0, v0}, Landroid/graphics/Canvas;->drawBitmap(Landroid/graphics/Bitmap;FFLandroid/graphics/Paint;)V

    return-object v7

    .line 75
    :cond_7
    sget-object p0, Lcom/umeng/social/tool/ComposeTool;->direction:Lcom/umeng/social/tool/ComposeTool$ComposeDirection;

    sget-object p2, Lcom/umeng/social/tool/ComposeTool$ComposeDirection;->RIGHTUP:Lcom/umeng/social/tool/ComposeTool$ComposeDirection;

    if-ne p0, p2, :cond_8

    sub-int/2addr v1, v3

    int-to-float p0, v1

    .line 76
    invoke-virtual {v8, p1, p0, v9, v0}, Landroid/graphics/Canvas;->drawBitmap(Landroid/graphics/Bitmap;FFLandroid/graphics/Paint;)V

    return-object v7

    .line 77
    :cond_8
    sget-object p0, Lcom/umeng/social/tool/ComposeTool;->direction:Lcom/umeng/social/tool/ComposeTool$ComposeDirection;

    sget-object p2, Lcom/umeng/social/tool/ComposeTool$ComposeDirection;->RIGHTDOWN:Lcom/umeng/social/tool/ComposeTool$ComposeDirection;

    if-ne p0, p2, :cond_9

    sub-int/2addr v1, v3

    int-to-float p0, v1

    sub-int/2addr v2, v4

    int-to-float p2, v2

    .line 78
    invoke-virtual {v8, p1, p0, p2, v0}, Landroid/graphics/Canvas;->drawBitmap(Landroid/graphics/Bitmap;FFLandroid/graphics/Paint;)V

    :cond_9
    return-object v7
.end method
